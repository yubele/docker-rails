...
from=$(cat Dockerfile | grep FROM | sed -s 's/FROM debian://')
node=$(cat Dockerfile | grep 'ENV NODE_VERSION' | sed -s 's/ENV NODE_VERSION=//' | sed -s 's/"//g' )
...
echo $from-node$node-npm$npm-ruby$ruby-bundler$bundler
